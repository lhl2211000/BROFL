...
class Data:
    def __init__(
        ...
        #If num_train_samples cannot be evenly divided by num_users, allocate the excess samples to the first client.
        data_split_sequence = []
        ...
        # Only partition into user datasets if we don't want centralized learning
            ...
            if self.alpha is None:
                # equal distribution 数据集随机划分给user，每个user样本数均等
    ...
    def split_data_dirichlet(self, dataset_train, visualize):
        ...
        for i in range(self.num_users): #生成一个二维数组，每个user都分配10个类的概率
        ...
        for c in range(self.num_classes):
            num_pts_per_user = (scaled_props[:, c] * len(class_idxs[c])).astype(
                int
            )  # giving each user a number of 类别为c的样本 based on their sampled proportion
    ...
    def _visualize_heterogeneity(self, num_samples_per_user_per_class):   #可视化异构性气泡图
